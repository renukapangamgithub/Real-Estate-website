...
const Header = () => {
  ...
  return (
    <nav className={`${darkMode ? "dark bg-black" : 'bg-[#f3f3f3]'} flex justify-between items-center gap-4 lg:px-20 px-4 py-3 sticky top-0 z-30`}>
      <div id="logo">
      ...
      <div className="flex justify-center items-center lg:hidden" onClick={toggleMenu}>
        <div>
          {isMenuOpen ? (
            <FaXmark aria-label="Close menu" className="text-black dark:text-white text-2xl cursor-pointer" />
          ) : (
            <FaBars aria-label="Open menu" className="text-black dark:text-white text-2xl cursor-pointer" />
          )}
        </div>
      </div>
      ...
      <div className="flex justify-center items-center lg:gap-8 gap-2">
            <div className="flex justify-center items-center lgz;gap-3 gap-1">
                <FaPhoneAlt className="size-5 text-red-600"/>
                <h1 className="lg:text-xl text-sm text-black dark:text-white font-semibold">930 833 7770</h1>
            </div>
